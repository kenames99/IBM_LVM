...
#include "..\..\jfs\common\include\jfs_cntl.h"   /* JFSCTL_QUIESCE, JFSCTL_RESUME */
...
void Discover_Volumes( CARDINAL32 * Error_Code )
{
  ...
  for ( Index = 0; Index < DriveCount; Index++ )
  {
    ...
    if ( DriveArray[Index].Is_PRM && ( ! DriveArray[Index].Corrupt ) && ( ! DriveArray[Index].Unusable ) )
    {
  ...
  if ( PartitionRecord->Partition_Type == Partition )
  {
    ...
    if ( PartitionRecord->Volume_Handle == NULL )
    {
      ...
      do
      {
        ...
        if ( DriveArray[PartitionRecord->Drive_Index].Is_Big_Floppy )
          sprintf(Name, "[ PRM %lu ]", *Name_Count);
        else
          sprintf(Name, "[ V %lu ]", *Name_Count);
