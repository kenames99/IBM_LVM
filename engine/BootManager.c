...
void Commit_Boot_Manager_Changes( CARDINAL32 * Error_Code )
{
  ...
  if ( Upgrade_Boot_Manager )
  {
    ...
    /* Now we must tell Boot Manager where to find himself.  Boot Manager will normally occupy only part of a single track
       on the disk.  However, for drives with small tracks ( less than 32 sectors per track ), Boot Manager may not fit on
       a single track.  In this case, Boot Manager will spill over into the following tracks.

       Boot Manager keeps track of himself through a table embedded in the Boot Manager Boot Sector.  In the normal case,
       only the first entry is used as Boot Manager is contained in a single track.  But for drives with small tracks,
       additional table entries are used to point to the extra tracks that Boot Manager spills into.  We must calculate
       all of the entries that may be used in this table and update the table before we write Boot Manager to disk.         */
